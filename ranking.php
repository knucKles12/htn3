...
switch($action) 
{
  ...
  $layout->createlayout_top('HackTheNet - Rangliste');
  
  $updtime=$gres->nicetime((int)@$get->get_file('data/calc-time.dat'));
  ...
  if($type=='user') 
  {
    ...
    else $udat=$get->get_user($uid);
    
    $rank=$udat['rank'];
    if($start==0) $start=$rank-4;
    if($start<=0) $start=1;
    
    #$total=(int)$get->get_file('data/_server'.$server.'/rank-user-count.dat");
    $info=$get->get_tableinfo('rank_users',$dbc->dbname($server));
    $total=$info[Rows];
    $r=$dbc->db_query('SELECT * FROM rank_users WHERE platz>='.mysql_escape_string($start).' AND platz<'.mysql_escape_string($start).'+10;');
  ...
  else 
  {
    ...
    $r=$dbc->db_query('SELECT * FROM `rank_clusters` ORDER BY `'.mysql_escape_string($sts[$sorttype]).'` DESC;');
  ...
  switch($_REQUEST['type']) 
  {
    ...
    $u=$get->get_User($name,'name');
    
    if($u!==false)
    {
      if($gres->is_noranKINGuser($u['id']))
      {
    ...
    {
      $gres->simple_message('Einen Benutzer mit diesem Name gibt es nicht!');
    }
    break;
    
    case 'cluster':
    $c=$get->get_cluster(trim($_REQUEST['cluster']),'code');
    ...
    {
      $gres->simple_message('Einen Cluster mit diesem Code gibt es nicht!');
    }
    break;
