...
class get
{
  ...
  public function __construct() {
    ...
    $this->database = $this->persistence->get_db();
  }
  ...
  function get_Cluster($value, $by = 'id') {
    $result = $this->persistence->db_query('SELECT * FROM clusters WHERE ' . mysqli_real_escape_string($this->database, $by) .
    ' LIKE \'' . mysqli_real_escape_string($this->database, $value) . '\' LIMIT 1');
    return ($result->num_rows > 0 ? mysqli_fetch_assoc($result) : false);
  }

  function get_news() {
    $result = $this->persistence->db_query('SELECT * FROM news ORDER BY Datum DESC');
    return ($result->num_rows > 0 ? mysqli_fetch_assoc($result) : false);
  }

  function get_pwd($id) {
    $result = $this->persistence->db_query('SELECT * FROM users WHERE id=' . mysqli_real_escape_string($this->database, $id) . '');
    return ($result->num_rows > 0 ? mysqli_fetch_assoc($result) : false);
  }

  function get_User($value, $by = 'id') {
    $value = str_replace('_', '\_', $value);
    $value = str_replace('%', '\%', $value);
    $result = $this->persistence->db_query('SELECT * FROM users WHERE ' . mysqli_real_escape_string($this->database, $by) .
    ' LIKE \'' . mysqli_real_escape_string($this->database, $value) . '\' LIMIT 1');
    return ($result->num_rows > 0 ? mysqli_fetch_assoc($result) : false);
  }
  ...
  function get_PC($id, $by = 'id') {
    $result = $this->persistence->db_query('SELECT * FROM pcs WHERE ' . mysqli_real_escape_string($this->database, $by) .
    ' LIKE \'' . mysqli_real_escape_string($this->database, $id) . '\' LIMIT 1');
    return ($result->num_rows > 0 ? mysqli_fetch_assoc($result) : false);
  }
  ...
  function get_PCPoints($pc, $mode = 'byid') {
    ...
    if ($mode == 'byid')
      $pcdat = mysqli_fetch_assoc($this->persistence->db_query('SELECT * FROM pcs WHERE id=\'' . mysqli_real_escape_string($this->database, $pc) . '\''));
    else
      $pcdat = $pc;
    $pcpoints = 0;
