...
class abook
{
    function list_items($g)
    {
        ...
        echo '<th>Benutzer</th><th>Wählen</th></tr>';
        ...
        while ($entry = mysql_fetch_object($r))
        {
            echo '<tr><td><a href="user.php?a=info&sid=' . $sid . '&user=' . $entry->uid .
                '">' . $entry->u_name . '</a></td>';
            echo '<td><input type="button" value="Wählen" onclick="choose(\'' .
    ...
    function admin_list($ix)
    {
        ...
        if (mysql_num_rows($r) > 0)
        {
            echo '<form action="abook.php?sid=' . $sid . '&amp;m=saveadmin&amp;group=' . $ix .
                '" method="post">
        <table>
        <tr><th>User</th><th>Gruppe</th><th>Löschen?</th></tr>';
            ...
            echo '<tr><td colspan="3" align="right"><input type="submit" value="Ausführen" /></td></tr></table></form>';
        }
    }
