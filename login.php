...
if ($ref != '' && $refd['host'] != $_SERVER['HTTP_HOST'] && $refd['host'] !=
    'ssl-id1.de' && count($_GET) < 2)
...
if ($action == 'login')
{
  ...
  <input type="submit" value="Ignorieren" />
    ...
    $pwd = trim($_POST['pwd']);
    $postpwd = $pwd;
    $usrname = trim($_POST['nick']);
    $server = (int)$_REQUEST['server'];
    if ($server != 1 && $server != 2)
    {
        $gres->simple_message('Falsche Server-Angabe!');
        exit;
    }

    $cookie = false;

    if (substr_count(($_COOKIE['htnLoginData4']), '|') == 2 && $_POST['save'] ==
        'yes')
    {
    ...
    if ($_POST['save'] == 'yes')
    {
        if ($cookie == false)
        {
            setcookie('htnLoginData4', $server . '|' . $usrname . '|' . md5($pwd), time() +
                10 * 365 * 24 * 60 * 60);
        } else
        {
            if ($usrnamedummy != $usrname || $postpwd != '[xpwd]')
                $pwd = md5($postpwd);
            setcookie('htnLoginData4', $server . '|' . $usrname . '|' . $pwd, time() + 10 *
                365 * 24 * 60 * 60);
            $cookie = true;
        }
    } else
    ...
    if (preg_match("/^[a-z0-9]+$/i", $pwd) == false || strlen($pwd) != 32 ||
        preg_match("/(_|%)/", $pwd))
        die('FUCK OFF!');

    $r = $dbc->db_query('SELECT * FROM users WHERE name=\'' . mysql_escape_string($usrname) .
        '\' LIMIT 1;');
    ...
    if (@mysql_num_rows($r) == 1)
    {
        ...
        if ($usr['password'] != $pwd)
        {
            $gres->simple_message('Passwort für diesen Account falsch!');
        ...
        if ($server == $t_limit_server && time() < $ts_server_start && $usr['stat'] !=
            1000)
        {
            $gres->simple_message('Server ' . $t_limit_server . ' startet erst ' . $gres->
                nicetime($ts_server_start) . '!');
            exit;
        }
        ...
        if (count($_GET) < 2 || $usr['bigacc'] == 'yes')
        {
            ...
            if ($usr['locked'] == 'yes' && ($usr['locked_till'] > time() || $usr['locked_till'] ==
                0))
            {
                if ($usr['locked_till'] == 0)
                    $time = 'auf unbegrenzte Zeit';
                else
                    $time = ' bis ' . $gres->nicetime($usr['locked_till']);
    ...
    du die genauen Gr&uuml;nde wissen willst, schick eine Email an <a href="mailto:' .
            ...
            $sid = $server . $gres->random_string(9);
            ...
            @$dbc->db_query('INSERT INTO logins ( ip, usr_id, time, user_agent ) VALUES ( \'' .
                mysql_escape_string($ip2) . '\', \'' . $usrid . '\', \'' . mysql_escape_string(time
                ()) . '\', \'' . mysql_escape_string($_SERVER['HTTP_USER_AGENT']) . '\' );');
            $dbc->db_query('UPDATE users SET sid=\'' . mysql_escape_string($sid) . '\', login_time=\'' .
                time() . '\', sid_ip=\'' . mysql_escape_string($ip) . '\', noipcheck=\'' . $noipcheck .
                '\', last_verified=0, sid_pc=' . $pcid . ', sid_lastcall=' . time() .
                ' WHERE id=\'' . $usrid . '\' LIMIT 1;');
            if ($gres->is_noranKINGuser($usrid) === false)
            {
                $s = 'data/_server' . $server . '/logins_' . strftime('%x') . '.txt';
                $get->put_file($s, ((int)@$get->get_file($s)) + 1);
            }
    ...
    {
        $gres->simple_message('Benutzername unbekannt! Stelle sicher dass du deinen Account aktiviert hast!');
    }
...
} elseif ($action == 'logout')
{
    ...
    if ($usr !== false)
    {
        ...
        $dbc->db_query('UPDATE users SET sid=\'\', sid_lastcall=0 WHERE id=\'' . $usr['id'] .
            '\' AND sid=\'' . $sid . '\'');
    }
    if ($_GET['redir'] == 'forum')
    {
