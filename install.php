...
include('config.php');
if ($htn_installed) {
  echo "<br>Wenn du HTN 2.1 neu installieren willst l&�uml;sche bitte diese Zeile aus deiner config.php:<br><i>\$htn_installed = true;</i>";
	exit;
}
...
<body text="#000000" bgcolor="#FFFFFF" link="#000080" alink="#000080" vlink="#000080">
  ...
  <!-- Start Navi -->
   <ul class="navigation">
    ...
    <li>
     ...
     <div class="help">In diesem Schritt werden verschiedene Einstellungen gepr&uuml;ft.<div>
    </li>
    <!-- Schritt 3 -->
    <li>
     <a href="#" title="">
     ...
     <div class="help">In diesem Schritt <i>m&uuml;ssen</i> die Daten angeben, mit denen HTN 3 sp&auml;ter laufen wird.</div>
    </li>
    <!-- Schritt 4 -->
    <li>
     <a href="#" title="">
  ...
  switch ($step) {
      ...
      echo '   <h3>Einstellungen pr&�uml;fen</h3>'."\n";
      if ($phpversion[0]==5) {
      ...
      **/
      ...
      switch ($do) {
          default:
          /**
               * Erstellt eine Datenbank und f&�uml;llt diese
               **/
...
mysql_query("CREATE TABLE `verify_imgs` (
  ...
  KEY `id` (`id`)
          ...
          if (!mysql_error())
          {
              echo '   <h3>Datenbank wurde erstellt und gef&�uml;llt</h3>'."\n";
...
\$no_ranking_cluster['server1'] = 1; // Nur eine Angabe m�glich f�r ID des Admin-Clusters
...
define('UPDATE_INTERVAL', \$name['update_int']); # Interval f�r Punkte-Updates in Sekunden
define('MIN_ATTACK_XDEFENCE', \$name['min_attack_def']);
define('MIN_INACTIVE_TIME', \$name['min_incative']); # Inaktive Zeit vor m�glichem Angriff
