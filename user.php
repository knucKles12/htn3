...
switch ($action)
{
    case 'config': //------------------------- CONFIG -------------------------------
        ...
        $layout->createlayout_top('HackTheNet - Optionen');
  ...
  <h2>Optionen</h2>';
        ...
        {
            $usr[$bez] = $gres->safeentities(html_entity_decode($val));
        }
        ...
        if ($usr['bigacc'] == 'yes')
            $account = 'Extended Account ' . ($usr['extacc_id'] != '' ?
                '(<span style="color:red;">Deine ExtAcc-ID: ' . $usr['extacc_id'] .
                '</span>)<br />
  Diese ID bitte gut aufbewahren! Ohne sie kannst du deinen ExtAcc in kommenden Runden/Versionen von HTN nicht mehr benutzen!' :
        ...
        {
            if (($data['bigacc'] == 'yes' && $usr['bigacc'] == 'yes') || $data['bigacc'] ==
                'no')
            {
        ...
        if ($xsinfo != '')
            $xsinfo = '<br />Weitere Stylesheets in Extended Accounts: ' . substr($xsinfo, 0,
                strlen($xsinfo) - 2);
  ...
  <a href="abook.php?mode=admin&amp;sid=' . $sid . '">Adressbuch verwalten</a>
        ...
        if ($usr['bigacc'] == 'yes')
        {
            ...
            $url = 'http://' . $tmp['host'] . $tmp['path'] . '/usrimg.php/' . $server . '-' .
                $usrid . '.png';
            $url = str_replace('//', '/', $url);
  ...
  <th>Geburtsdatum:</th>
  <td><select name="bday">' . $days . '</select>. <select name="bmonth">' . $months .
            '</select> <select name="byear">' . $years . '</select></td>
  </tr>
  ...
  <td><input type="text" name="homepage" value="' . $usr['homepage'] .
            '" maxlength="100" /></td>
  </tr>
  ...
  <td><textarea name="aboutme" rows="5" cols="50">' . $usr['infotext'] .
            '</textarea></td>
  </tr>
  <tr id="settings-settings-avatar">
  <th>Avatar-Bild (http://&nbsp;...):</th>
  <td><input type="text" name="avatar" value="' . $usr['avatar'] . '" />' . $avatar .
            '</td>
  </tr>
  ...
  <td>' . $usrimg . '</td>
        ...
        $usrimg_fmt = '';
        $fmts = array('points', 'ranking', 'points ranking', 'cluster points',
            'cluster ranking', 'cluster points ranking');
        $fmtnms = array('Punkte', 'Ranglisten-Platz', 'Punkte + Platz',
            'Cluster + Punkte', 'Cluster + Platz', 'Cluster + Platz + Punkte');
  ...
  <td><input name="apwd" type="password" maxlength="16" /></td>
  </tr>
  <tr id="settings-password-password">
  ...
  <td><input name="pwd2" type="password" maxlength="16" /></td>
  </tr>
  ...
  ';
    ...
    <hr>
            ...
            if (!@mail($usr['email'], 'HackTheNet-Account löschen?', $body,
                'From: robot@hackthenet.org'))
            {
                echo nl2br($body);
            }

            $get->put_file('data/regtmp/del_account_' . $code . '.txt', $usrid . '|' . $server);
            @unlink('data/login/' . $sid . '.txt');
        } else
        {
            # Nicht Account löschen sondern Settings speichern
            ...
            if ($e == false)
            {
                ...
                $usr['homepage'] = $gres->safeentities($hp);
                $usr['infotext'] = $gres->safeentities($text);
                $usr['wohnort'] = $gres->safeentities($ort);
                $usr['sig_mails'] = $gres->safeentities($sig_mails);
                $usr['sig_board'] = $gres->safeentities($sig_board);
                $usr['inbox_full'] = $gres->safeentities($inbox_full);
                $usr['avatar'] = $gres->safeentities($avatar);
    ...
    case 'setmailaddy': //------------------------- SET MAIL ADDY -------------------------------
        ...
        } else
        {
            ...
            {
                $dbc->db_query('UPDATE users SET email=\'' . mysql_escape_string($email) . '\' WHERE id=\'' .
                    $usrid . '\'');
                echo mysql_error();
                header('Location: user.php?a=config&sid=' . $sid . '&saved=1');
            } else
            {
                $gres->simple_message('Falsches Passwort!');
            }
        }
        break;
    ...
    case 'info': //------------------------- INFO -------------------------------
        $index = $_REQUEST['user'];
        $a = $get->get_user($index);
        if ($a != false)
        {

            $u_points = $a['points'];
            $layout->createlayout_top('HackTheNet - Benutzerprofil');
            if ($a['gender'] == 'x')
                $geschl = '';
            elseif ($a['gender'] == 'm')
            ...
            elseif ($a['gender'] == 'w')
                $geschl = 'Weiblich';
            if ($geschl != '')
                $geschl = '<tr>' . LF . '<th>Geschlecht:</th>' . LF . '<td>' . $geschl . '</td>' .
                    LF . '</tr>' . "\n";
            if ($a['wohnort'] != '')
                $ort = '<tr>' . LF . '<th>Wohnort:</th><td>' . $a['wohnort'] . '</td>' . LF .
                    '</tr>' . "\n";

            if ($a['locked'] == 'yes' && ($a['locked_till'] > time() || $a['locked_till'] ==
                0))
            {
                if ($a['locked_till'] == 0)
                    $lotime = 'auf unbegrenzte Zeit';
                else
                    $lotime = ' bis ' . nicetime($a['locked_till']);
                $locked = '<tr id="account-locked">' . LF . '<th>Besonderheiten:</th>' . LF .
                    '<td>Account gesperrt ' . $lotime . '</td>' . LF . '</tr>' . LF;
            }
            ...
            if ($a['birthday'] != '0.0.0')
            {
                ...
                {
                    $alter = $years . ' Jahre';
                    $gb = '<tr>' . LF . '<th>Alter</th>' . LF . '<td>' . $alter . '</td>' . LF .
                        '</tr>' . "\n";
                }
            }
            if (eregi('http://.*', $a['homepage']) != false)
            {
                $hp = $gres->dereferurl($a['homepage']);
                $hp = $gres->safeentities($hp);
                $hp = '<tr>' . LF . '<th>Homepage:</th><td><a href="' . $hp . '">' .
                    safeentities($a['homepage']) . '</a></td>' . LF . '</tr>' . "\n";
            }
            ...
            {
                $c = $get->get_cluster($c);
                $scluster = '<a href="cluster.php?a=info&amp;cluster=' . $a['cluster'] .
                    '&amp;sid=' . $sid . '">' . $c['name'] . '</a> ' . $c['code'];
            } else
                $scluster = 'keiner';

            $spcs = '';
            $sql = $dbc->db_query('SELECT * FROM pcs WHERE owner=' . mysql_escape_string($a['id']) .
                ' ORDER BY name ASC;');
            $pccnt = mysql_num_rows($sql);
            while ($xpc = mysql_fetch_assoc($sql))
            {
                $country = $get->Get_Country('id', $xpc['country']);
                $xpc['name'] = htmlentities($xpc['name']);
                if ((int)$usr['stat'] >= 100)
                    $extras = ' <a href="secret.php?sid=' . $sid . '&amp;m=file&amp;type=pc&amp;id=' .
                        $xpc['id'] . '">Extras</a>';
                else
                    $extras = '';
                $spcs .= '<li>' . $xpc['name'] . ' (10.47.' . $xpc['ip'] .
                    ', <a href="game.php?m=subnet&amp;sid=' . $sid . '&amp;subnet=' . $gres->
                    subnetfromip($xpc['ip']) . '">' . $country['name'] . '</a>, ' . number_format($xpc['points'],
                    0, ',', '.') . ' Punkte)' . $extras . '</li>';
            }
            ...
            if ($usr['stat'] >= 100)
                $descr .= '</td>' . LF . '</tr>' . LF . '<tr>' . LF .
                    '<th>Sonder-Funktionen:</th>' . LF . '<td><a href="secret.php?sid=' . $sid .
                    '&amp;m=file&amp;type=user&amp;id=' . $a['id'] . '">' . ($usr['stat'] == 1000 ?
                    'Bearbeiten' : 'Daten ansehen') . '</a>';
            ...
            if ($usr['stat'] == 1000)
            {
                ...
                if ($a['locked'] == 'yes' && ($a['locked_till'] > time() || $a['locked_till'] ==
                    0))
                {
                    $descr .= '<a href="secret.php?a=unlockacc&amp;sid=' . $sid . '&amp;user=' . $a['id'] .
                        '">Account <b>ent</b>sperren</a>';
                } else
                {
                    $descr .= '<a href="secret.php?a=lockacc&amp;sid=' . $sid . '&amp;user=' . $a['id'] .
                        '">Account schnell sperren</a> | ';
                    $descr .= '<a href="secret.php?a=lockaccex&amp;sid=' . $sid . '&amp;user=' . $a['id'] .
            ...
            if (eregi('http://.*/.*', $a['avatar']) !== false)
            {
                ...
                {
                    $avatar = $a['avatar'];
                    $avatar = '<tr><td colspan="2"><img src="' . $avatar . '" alt="' . $a['name'] .
                        '" /></td></tr>';
                    #
                }
            }
    ...
    <h3>' . $a['name'] . '</h3>
    <div class="submenu">
    <p><a href="mail.php?m=newmailform&amp;sid=' . $sid . '&amp;recip=' . $a['name'] .
                '">Mail an User</a> |
    <a href="ranking.php?m=ranking&amp;sid=' . $sid . '&amp;type=user&amp;id=' .
                $a['id'] . '">User in Rangliste</a>
    ' . $bigacc . '</p>
    </div>
    <table>
    ' . $avatar . '
    ...
    ';
            ...
            $gres->simple_message('Diesen Benutzer gibt es nicht!');
        break;

    case 'newpwd': //------------------------- NEW PWD -------------------------------
        $pwd = trim($_POST['pwd']);
        $pwd2 = trim($_POST['pwd2']);
        $apwd = md5(trim($_POST['apwd']));
        $oldpwd = $get->get_pwd($usrid);

        if ($oldpwd['password'] == $apwd)
        {
            if ($pwd == $pwd2)
            {
